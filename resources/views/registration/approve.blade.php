    ...
    <div class="content container-fluid">
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <h3 class="page-title">Approved Student</h3>
                    {{-- <ul class="breadcrumb">
                        ...
                        <div class="tab-pane fade show" id="approved" role="tabpanel">
                            <div class="table-responsive">
                                <table class="table table-hover table-center mb-0 datatable">
                                    ...
                                    <tbody>
                                    @if(count($reg_approved) > 0)
                                        @foreach ($reg_approved as $key => $row)
                                            <tr>
                                                {{--                                                <td>{{ $key + 1 }}</td>--}}
                                                <td>{{ ($reg_approved->currentPage() - 1) * $reg_approved->perPage() + $key + 1 }}</td>
                                                <td>{!! ucfirst($row->name) . ' S/o Mr. ' . ucfirst($row->father_name) !!}</td>
                                                <td>{!! $row->email !!}</td>
                                                <td>{!! date('F d, Y', strtotime($row->dob)) !!}</td>
                                                <td>{!! $row->mobile !!}</td>
                                               <td>{!! $row->amount !!} <b class="badge bg-success">{!! $row->payment_status !!}</b></td>
                                ...
                                @if (count($reg_approved) != 0)
                                    <div class="p-2">
                                        {!! $reg_approved->links('pagination::bootstrap-5') !!}
                                    </div>
                                @endif
                            </div>
                        </div>
